...
Another way would be the configuration of `SSH_ARGS` in `/storage/.cache/services/sshd.conf`.
But this file gets overwritten by the `service.libreelec.settings` addon.
...
Reboot the system if you are confident that everything is configured correctly.
To check if sshd started with your arguments run: `ps | grep sshd`

Output should show something similar to this:
